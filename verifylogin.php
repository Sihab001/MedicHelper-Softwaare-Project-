...
///database connection 
    ...
    if(isset($_POST["submit"])){
        ...
        $userquery1= "select * from doctor where username='$username' AND password='$password' ";
        $returnvalue1=$conn->prepare($userquery1);
        $returnvalue1->execute();
        $rowcount1 = $returnvalue1->rowCount();
        if($rowcount==1){
            if(!isset($_SESSION)){
                session_start();
            }
